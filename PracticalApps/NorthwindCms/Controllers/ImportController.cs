    ...
    public class ImportController : Controller
    {
        ...
        public async Task<IActionResult> Import()
        {
            ...
            foreach (var category in db.Categories.Include(c => c.Products))
            {
                ...
                // the skip to the next iteration of the loop
                CategoryPage categoryPage = await api.Pages.GetBySlugAsync<CategoryPage>($"catalog/{category.CategoryName.ToLower()}");
                ...
                if (categoryPage == null)
                {
                    ...
                    var image = (await api.Media.GetAllByFolderIdAsync())
                        .First(media => media.Type == MediaType.Image && media.Filename == $"category{category.CategoryID}.jpeg");
